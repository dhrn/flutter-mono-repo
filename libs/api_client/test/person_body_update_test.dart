import 'package:api_client/model/person_body_update.dart';
import 'package:test/test.dart';

// tests for PersonBodyUpdate
void main() {
  final instance = PersonBodyUpdate();

  group(PersonBodyUpdate, () {
    // String firstName
    test('to test the property `firstName`', () async {
      // TODO
    });

    // String lastName
    test('to test the property `lastName`', () async {
      // TODO
    });

    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // String email
    test('to test the property `email`', () async {
      // TODO
    });

    // String skypeId
    test('to test the property `skypeId`', () async {
      // TODO
    });

    // String googleId
    test('to test the property `googleId`', () async {
      // TODO
    });

    // String instantMessageId
    test('to test the property `instantMessageId`', () async {
      // TODO
    });

    // String jobTitle
    test('to test the property `jobTitle`', () async {
      // TODO
    });

    // String location
    test('to test the property `location`', () async {
      // TODO
    });

    // Company company
    test('to test the property `company`', () async {
      // TODO
    });

    // String mobile
    test('to test the property `mobile`', () async {
      // TODO
    });

    // String telephone
    test('to test the property `telephone`', () async {
      // TODO
    });

    // String userStatus
    test('to test the property `userStatus`', () async {
      // TODO
    });

    // bool enabled
    test('to test the property `enabled`', () async {
      // TODO
    });

    // bool emailNotificationsEnabled
    test('to test the property `emailNotificationsEnabled`', () async {
      // TODO
    });

    // String password
    test('to test the property `password`', () async {
      // TODO
    });

    // String oldPassword
    test('to test the property `oldPassword`', () async {
      // TODO
    });

    // BuiltList<String> aspectNames
    test('to test the property `aspectNames`', () async {
      // TODO
    });

    // JsonObject properties
    test('to test the property `properties`', () async {
      // TODO
    });
  });
}
